 ...
 *  banners on StoryDetail/VideoDetail and the "My Drafts" list on /me. Only
 *  the trigger + dialog JSX is shared — each caller keeps its own delete
 *  fetch/navigation logic, since that differs (MeView stays on the page and
 *  refreshes its list; the detail pages navigate back to /me). */
export function DeleteDraftDialog({ slug, disabled, onDelete, compact }: DeleteDraftDialogProps) {
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button
          ...
          disabled={disabled}
          aria-label={compact ? "Delete draft" : undefined}
        >
